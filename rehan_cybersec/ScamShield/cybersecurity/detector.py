...
PATTERNS: list[PatternSpec] = [
    ...
    ("payment_request", [r"\b(?:pay|payment|fee|charges?|deposit|transfer)\b.{0,20}(?:rs\.?|inr|₹|\d)", r"(?:rs\.?|inr|₹)\s?\d+.{0,25}\b(?:pay|payment|fee|deposit|charges?)\b"]),
    ("money_transfer_request", [r"\b(?:transfer|send)\b.{0,30}\b(?:money|amount|funds?|cash|₹|rs\.?|inr)\b", r"\b(?:bank transfer|wire transfer|upi transfer)\b"]),
...
def detect_indicators(text: str | None) -> list[dict[str, Any]]:
    ...
    if "qr" in lowered and re.search(r"\b(?:scan|pay|payment|upi|receive money|collect)\b", lowered):
